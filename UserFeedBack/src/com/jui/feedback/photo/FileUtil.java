package com.jui.feedback.photo;

import java.io.File;
import java.io.FileNotFoundException;
import java.io.FileOutputStream;
import java.io.IOException;

import android.graphics.Bitmap;
import android.os.Environment;
import android.util.Log;

public class FileUtil
{

    public static String SDPATH = Environment.getExternalStorageDirectory( ) + "/formats/";

    public static void saveBitmap( Bitmap bm , String picName )
    {
	try
	{
	    createSDDir( );

	    File f = new File( SDPATH , picName + ".JPEG" );
	    if( f.exists( ) )
	    {
		f.delete( );
	    }
	    FileOutputStream out = new FileOutputStream( f );
	    bm.compress( Bitmap.CompressFormat.JPEG , 90 , out );
	    out.flush( );
	    out.close( );
	    Bimp.paths.add( SDPATH + picName + ".JPEG" );//临时文件路径
	}
	catch ( FileNotFoundException e )
	{
	    Log.i( "berrytao" , "保存文件出错," + e.getMessage( ) );
	}
	catch ( IOException e )
	{
	    Log.i( "berrytao" , "保存文件出错," + e.getMessage( ) );
	}
    }

    public static void createSDDir() throws IOException
    {
	File dir = new File( SDPATH );
	if( Environment.getExternalStorageState( ).equals( Environment.MEDIA_MOUNTED ) )
	{
	    if( !dir.exists( ) )
	    {
		dir.mkdirs( );
		Log.i( "berrytao" , "文件夹创建成功" );
	    }
	}

    }

    private static boolean isFileExist( String fileName )
    {
	File file = new File( SDPATH + fileName );
	file.isFile( );
	return file.exists( );
    }

    public static void delFile( String fileName )
    {
	Bimp.paths.remove( SDPATH + fileName + ".JPEG" );
	File file = new File( SDPATH + fileName + ".JPEG" );
	if( file.isFile( ) && isFileExist( fileName + ".JPEG" ) )
	{
	    file.delete( );
	}

    }

    public static void deleteDir()
    {
	Bimp.paths.clear( );
	File dir = new File( SDPATH );
	if( dir == null || !dir.exists( ) || !dir.isDirectory( ) )
	    return;

	for( File file : dir.listFiles( ) )
	{
	    if( file.isFile( ) )
		file.delete( ); // 删除所有文件
	    else if( file.isDirectory( ) )
		deleteDir( ); // 递规的方式删除文件夹
	}
	dir.delete( );// 删除目录本身
    }

    public static boolean fileIsExists( String path )
    {
	try
	{
	    File f = new File( path );
	    if( !f.exists( ) )
	    {
		return false;
	    }
	}
	catch ( Exception e )
	{

	    return false;
	}
	return true;
    }

}
